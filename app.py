...
with tab3:
    ...
    with col_g2:
        ...
        # Filtra os dados removendo '-' e 'Não Informado' ANTES de contar
        df_clean_view = df_criticos[~df_criticos[opt_view].isin(['-', 'Não Informado'])]
        
        df_view = df_clean_view[opt_view].value_counts().reset_index()
        ...
        if not df_view.empty:
            ...
            st.warning("Sem dados válidos (sem '-') para exibir neste gráfico.")

    # 4. TABELA E EXPORTAÇÃO
    st.markdown("### 📋 Lista de Fregueses (Casos Críticos)")
    
    if not df_criticos.empty:
        # Prepara a tabela completa para ordenação
        df_table = df_criticos[['ID_Ref', 'Episodios_Reais', 'Dias_Em_Aberto', 'Perfil_Cliente', 'Motivos_Lista', 'Transportadora']].copy()
        df_table = df_table.sort_values(by=['Episodios_Reais', 'Dias_Em_Aberto'], ascending=False)
        ...
        with col_dl1:
            ...
            st.download_button(
                ...
                data=csv,
                file_name='fregueses_reincidencia.csv',
                mime='text/csv',
            )
        
        # Exibe apenas os TOP 50 na tela para não travar o navegador, mas o download acima baixa TUDO
        max_val = int(df_criticos['Episodios_Reais'].max()) # Garante inteiro para evitar erro
        
        st.dataframe(
            df_table.head(50), # Limitado a 50 na visualização
            column_config={
        ...
        st.caption("Visualizando os top 50 casos mais críticos. Use o botão acima para baixar a lista completa.")
    else:
