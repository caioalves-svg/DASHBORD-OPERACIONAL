# ... (Mantenha o código anterior até a linha 'with tab3:')
...
with tab3:
    ...
    # ---------------------------------------------------------
    # 1. PREPARAÇÃO DOS DADOS (CÁLCULOS AVANÇADOS)
    # ---------------------------------------------------------
    # Agrupa por Pedido
    df_reincidencia = df_filtered.groupby('ID_Ref').agg(
        ...
        Motivos_Lista=('Motivo', lambda x: list(set(x))), # Lista de motivos
        Transportadora=('Transportadora', 'first'),       # Pega a transportadora (assume a mesma)
        Portal=('Portal', 'first')
    ).reset_index()
    ...
    # SEGMENTAÇÃO (O INSIGHT DE OURO)
    # Define categorias de clientes baseado no comportamento
    ...
    # Separando apenas os problemáticos (>1 episódio)
    df_criticos = df_reincidencia[df_reincidencia['Episodios_Reais'] > 1].copy()

    # ---------------------------------------------------------
    # 2. KPIS DE REINCIDÊNCIA (NOVO)
    # ---------------------------------------------------------
    ...
    # Custo Estimado (Simulação: Supondo 15min por atendimento a R$ 20,00/hora = R$ 5,00 por toque)
    # Contatos EXTRAS (Total Bruto - 1 contato ideal por pedido reincidente)
    contatos_excedentes = df_criticos['Total_Bruto'].sum() - df_criticos.shape[0]
    horas_desperdicadas = (contatos_excedentes * 15) / 60 # 15 min por contato
    ...
    # ---------------------------------------------------------
    # 3. VISUALIZAÇÃO GRÁFICA MELHORADA
    # ---------------------------------------------------------
    ...
    with col_g1:
        ...
        if not df_criticos.empty:
            # Scatter Plot Colorido pelo Perfil
            fig_scatter = px.scatter(
                df_criticos,
                x='Dias_Em_Aberto',
                y='Episodios_Reais',
                color='Perfil_Cliente', # AQUI ESTÁ A MÁGICA VISUAL
                size='Total_Bruto',
                hover_data=['ID_Ref', 'Motivos_Lista', 'Transportadora'],
                title="Perfil: Ansiosos (Esq/Alto) vs Crônicos (Dir/Baixo)",
                color_discrete_map={
            ...
            # Adiciona linhas de referência para dividir quadrantes
            ...
            fig_scatter.update_layout(
                height=450, 
                xaxis_title="Dias em Aberto (Duração)", 
                yaxis_title="Qtd de Episódios (Insistência)"
            )
    ...
    with col_g2:
        ...
        # Gráfico de quem causa mais reincidência (Transportadora ou Motivo)
        ...
        fig_bar_r = px.bar(
            ...
            y=opt_view, 
            orientation='h',
            text='Qtd Reincidentes',
            color='Qtd Reincidentes',
    ...
    # ---------------------------------------------------------
    # 4. TABELA ANALÍTICA (SUBSTITUI OS CARDS)
    # ---------------------------------------------------------
    ...
    if not df_criticos.empty:
        ...
        # Exibe com formatação nativa do Streamlit (Barras de progresso visual)
        st.dataframe(
            df_table,
            column_config={
                ...
                "Episodios_Reais": st.column_config.ProgressColumn(
                    "Episódios", 
                    help="Quantas vezes entrou em contato (janelas de 24h)",
                    format="%d",
                    min_value=0,
                    max_value=df_criticos['Episodios_Reais'].max()
                ),
